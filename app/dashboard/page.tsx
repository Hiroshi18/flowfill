import Nav from "@/components/Nav"

export default function Dashboard() {
  const classes = [
    { id: 1, name: "Morning Flow Yoga", time: "07:00", instructor: "Anna", capacity: 20, booked: 14 },
    { id: 2, name: "Power Pilates", time: "09:00", instructor: "Marc", capacity: 16, booked: 16 },
    { id: 3, name: "Midday Stretch", time: "12:00", instructor: "Sara", capacity: 20, booked: 5 },
    { id: 4, name: "Core & Breathe", time: "13:30", instructor: "Anna", capacity: 18, booked: 7 },
    { id: 5, name: "Vinyasa Flow", time: "16:00", instructor: "Tom", capacity: 20, booked: 12 },
    { id: 6, name: "Peak Hour Yoga", time: "18:00", instructor: "Sara", capacity: 20, booked: 20 },
    { id: 7, name: "Evening Pilates", time: "19:30", instructor: "Marc", capacity: 16, booked: 15 },
    { id: 8, name: "Late Flow", time: "20:30", instructor: "Tom", capacity: 14, booked: 4 },
  ]

  function getIncentive(booked: number, capacity: number) {
    const rate = booked / capacity
    if (rate < 0.4) return { label: "⭐ €5 credit", color: "bg-green-500/20 text-green-400 border border-green-500/30" }
    if (rate < 0.6) return { label: "⭐ €3 credit", color: "bg-amber-500/20 text-amber-400 border border-amber-500/30" }
    return null
  }

  function getFillColor(booked: number, capacity: number) {
    const rate = booked / capacity
    if (rate >= 0.9) return "bg-red-500"
    if (rate >= 0.6) return "bg-amber-500"
    return "bg-green-500"
  }

  const totalCapacity = classes.reduce((s, c) => s + c.capacity, 0)
  const totalBooked = classes.reduce((s, c) => s + c.booked, 0)
  const fillRate = Math.round((totalBooked / totalCapacity) * 100)
  const incentiveCount = classes.filter(c => getIncentive(c.booked, c.capacity)).length

  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      <Nav />
      <div className="p-8">
        <div className="max-w-5xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold">⚡ FlowFill Dashboard</h1>
            <p className="text-zinc-400 mt-1">Zenith Yoga Studio — Today's Schedule</p>
          </div>
          <div className="grid grid-cols-4 gap-4 mb-8">
            {[
              { label: "Overall Fill Rate", value: `${fillRate}%`, color: "text-indigo-400" },
              { label: "Classes Today", value: classes.length, color: "text-emerald-400" },
              { label: "Active Incentives", value: incentiveCount, color: "text-amber-400" },
              { label: "Empty Spots", value: totalCapacity - totalBooked, color: "text-rose-400" },
            ].map((stat, i) => (
              <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                <p className="text-zinc-500 text-sm mb-1">{stat.label}</p>
                <p className={`text-3xl font-bold ${stat.color}`}>{stat.value}</p>
              </div>
            ))}
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="p-5 border-b border-zinc-800">
              <h2 className="font-semibold text-lg">Class Schedule</h2>
            </div>
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-800">
                  {["Time", "Class", "Instructor", "Spots", "Fill Rate", "Incentive"].map(h => (
                    <th key={h} className="text-left text-zinc-500 text-xs uppercase tracking-wider px-5 py-3">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {classes.map((cls, i) => {
                  const incentive = getIncentive(cls.booked, cls.capacity)
                  const fillRate = Math.round((cls.booked / cls.capacity) * 100)
                  return (
                    <tr key={cls.id} className={`border-b border-zinc-800/50 hover:bg-zinc-800/30 transition ${i === classes.length - 1 ? "border-0" : ""}`}>
                      <td className="px-5 py-4 font-mono text-zinc-400 text-sm">{cls.time}</td>
                      <td className="px-5 py-4 font-semibold">{cls.name}</td>
                      <td className="px-5 py-4 text-zinc-400">{cls.instructor}</td>
                      <td className="px-5 py-4 text-zinc-400">{cls.booked}/{cls.capacity}</td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-24 bg-zinc-800 rounded-full h-1.5">
                            <div className={`h-1.5 rounded-full ${getFillColor(cls.booked, cls.capacity)}`} style={{ width: `${fillRate}%` }} />
                          </div>
                          <span className="text-sm text-zinc-400">{fillRate}%</span>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        {incentive ? (
                          <span className={`text-xs px-3 py-1 rounded-full font-semibold ${incentive.color}`}>{incentive.label}</span>
                        ) : (
                          <span className="text-zinc-600">—</span>
                        )}
                      </td>
                    </tr>
                  )
                })}
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
  )
}