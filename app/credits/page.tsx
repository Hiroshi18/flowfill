      ...
      <div className="p-8">
        <div className="max-w-5xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-zinc-400 mt-1">Zenith Yoga Studio — Customer credit balances</p>
          </div>
          <div className="grid grid-cols-4 gap-4 mb-8">
            {[
              { label: "Total Credits Issued", value: `€${totalCredits}`, color: "text-indigo-400" },
              { label: "Total Bookings", value: totalBookings, color: "text-emerald-400" },
              { label: "Attendance Rate", value: `${attendanceRate}%`, color: "text-amber-400" },
              { label: "Active Customers", value: customers.length, color: "text-rose-400" },
            ].map((stat, i) => (
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody>
                {customers.map((c, i) => (
                  <tr key={c.id} className={`border-b border-zinc-800/50 hover:bg-zinc-800/30 transition ${i === customers.length - 1 ? "border-0" : ""}`}>
                    <td className="px-5 py-4 font-semibold">{c.name}</td>
                    <td className="px-5 py-4 text-zinc-400 text-sm">{c.email}</td>
                    <td className="px-5 py-4">
                      <span className="bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 text-sm px-3 py-1 rounded-full font-semibold">
                        €{c.credits}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-zinc-400">{c.bookings}</td>
                    <td className="px-5 py-4">
                      <span className={`text-sm font-semibold ${Math.round(c.attended / c.bookings * 100) >= 80 ? "text-green-400" : "text-amber-400"}`}>
                        {c.attended} ({Math.round(c.attended / c.bookings * 100)}%)
                      </span>
                    </td>
                    <td className="px-5 py-4 text-zinc-400 text-sm">{c.lastClass}</td>
                  </tr>
                ))}
              </tbody>
            </table>
